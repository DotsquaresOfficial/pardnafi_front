...
const ContactUs = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        for (let key in contactUsField) {
            let checkContact = InputValid(key, contactUsField[key]);
            setContactUsFieldErr(prev => ({ ...prev, [key]: checkContact }));
            if (checkContact !== "") {
                return;
            }
        }
        ...
        const { firstName, contactNumber, email, message } = contactUsField;
        const data = { name: firstName, contactNumber, email, message };

        setLoading(true); 
     
        try {
            const result = await setContactUs(data);

            if (result.data.status) {
                toast.success(result.data.message);
                setContactUsField({ firstName: '', contactNumber: '', email: '', message: '' });
            } else {
                toast.error(result.data.message);
            }
        } catch (error) {
    ...
    <div className="section-header section-header--max50">
        <h2 className="mb-15 mt-minus-5">Contact Us</h2>
                ...
                <div className="col-md-7">
                    <div className="contact__form">
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <button type="submit" className="trk-btn trk-btn--border trk-btn--primary mt-4 d-block" disabled={loading}>
                                {loading ? <span className="loader"></span> : "Contact us now"}
                            </button>
                        </form>
                    </div>
                </div>
